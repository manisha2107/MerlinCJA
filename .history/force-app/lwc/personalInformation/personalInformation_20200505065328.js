...
export default class CreateRecordInLWC extends LightningElement {
    ...
    handleNameChange(event) {
        ...
        if(event.target.label === 'Enter Last Name'){

            this.accRecord.LastName = event.target.value;

        }
        if(event.target.label === 'Mailing Street'){

            this.accRecord.MailingStreet = event.target.value;

        }
        if(event.target.label === 'Enter Mailing Street'){

            this.accRecord.MailingStreet = event.target.value;

        }
        if(event.target.label === 'Enter Mailing City'){

            this.accRecord.MailingStreet = event.target.value;

        }
        if(event.target.label === 'Enter Mailing State'){

            this.accRecord.MailingState = event.target.value;

        }
        if(event.target.label === 'Enter Mailing PostalCode'){

            this.accRecord.MailingPostalCode = event.target.value;

        }
        if(event.target.label === 'Enter Mailing Country'){

            this.accRecord.MailingCountry = event.target.value;

        }
        if(event.target.label === 'Enter Email Address'){
